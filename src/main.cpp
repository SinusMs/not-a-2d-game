...
void InitializeAssetLoader() { BasePath = std::string{SDL_GetBasePath()}; }
...
LoadShader(SDL_GPUDevice *device, const std::string &shaderFilename,
  ...
  if (backendFormats & SDL_GPU_SHADERFORMAT_SPIRV) {
    fullPath = std::format("{}/Content/Shaders/Compiled/SPIRV/{}.spv", BasePath,
                           shaderFilename);
    format = SDL_GPU_SHADERFORMAT_SPIRV;
    entrypoint = "main";
  } else if (backendFormats & SDL_GPU_SHADERFORMAT_MSL) {
    fullPath = std::format("{}/Content/Shaders/Compiled/MSL/{}.msl", BasePath,
                           shaderFilename);
    format = SDL_GPU_SHADERFORMAT_MSL;
    entrypoint = "main0";
  } else if (backendFormats & SDL_GPU_SHADERFORMAT_DXIL) {
    fullPath = std::format("{}/Content/Shaders/Compiled/DXIL/{}.dxil", BasePath,
                           shaderFilename);
...
int main() {
  ...
  while (running) {
    ...
    if (swapchainTexture) {
      ...
      SDL_DrawGPUPrimitives(renderPass, 3, 1, 0,
                            0); // Draw a triangle with 3 vertices
